export const priceDataNails = [
  {
    id: 1,
    name: "Manicure Hybrydowy z Usunięciem Hybrydy",

    items: [
    
       {
        id: 1, name: "Manicure Hybrydowy + Jednolity Kolor", 
        description: "Inne Stylistki",
        price: 160, 
       },
      {
        id: 2, name: "Manicure Hybrydowy + Jednolity Kolor", 
        description: "Kasia",
        price: 170, 
      },
      {
        id: 3,
        name: "Manicure Hybrydowy + Zdobienie",
        description: "+ Zdobienie 2 szt, 4 szt, 6 szt",
        price: { min: 180, max:200 },
      },
      { id: 4, name: "Manicure Hybrydowy + French Hybrydą", price: 190 },
      { id: 5, name: "Utwardzenie Płytki Żelem", price: 20 },
      {
        id: 6,
        name: "Manicure Hybrydowy z Usunięciem + Peeling Rąk",
        price: 170,
      },
      {
        id: 7,
        name: "Manicure Hybrydowy + Ombre lub BabyBoomer",
        price: 190,
      },
    ],
  },
  {
    id: 2,
    name: "Manicure",

    items: [
      { id: 1, name: "Manicure + Odżywka", price: 100 },
      { id: 2, name: "Manicure z Pomalowaniem Zwykły Lakier", price: 130 },
      { id: 3, name: "Peeling Rąk + Masaż", price: 40 },
    ],
  },
  {
    id: 3,
    name: "Manicure Męski",
    items: [{ id: 1, name: "Manicure Męski", price: 100 }],
  },
  {
    id: 4,
    name: "Manicure Hybrydowy Czysta Płytka",
    items: [
      {
        id: 1,
        name: "Czysta Płytka Hybryda + Jednolity Kolor",
        price: 150,
      },
      {
        id: 2,
        name: "Hybryda + French",
        price: 180,
      },
      {
        id: 3,
        name: "Manicure hybrydowy + Ombre lub BabyBoomer",
        price: 180,
      },
      {
        id: 4,
        name: "Manicure hybrydowy + Zdobienie",
        description: "+ Zdobienie 2 szt, 4 szt, 6 szt",
        price: { min: 170, max: 190 },
      },
    ],
  },
  {
    id: 5,
    name: "Przedłużenie Paznokci, Paznokcie Żelowe",
    items: [
      {
        id: 1,
        name: "Żel na Szablonie + Hybryda Jednolity Kolor",
        price: 290,
      },
      {
        id: 2,
        name: "Żel na Szablonie + French Hybryda",
        price: 320,
      },
      {
        id: 3,
        name: "Żel na Szablonie + Ombre Hybryda",
        price: 320,
      },
    ],
  },
  {
    id: 6,
    name: "Uzupełnienie Żelu",
    items: [
      {
        id: 1,
        name: "Uzupełnienie Żelu + Hybryda Jednolity Kolor",
        price: 210,
      },
      {
        id: 2,
        name: "Uzupełnienie Żelu + Ombre",
        price: 230,
      },
      {
        id: 3,
        name: "Uzupełnienie Żelu + French",
        price: 230,
      },
      {
        id: 4,
        name: "Uzupełnienie Żelu + Zdobienie",
        price: { min: 230, max: 250 },
        description: "+ Zdobienie 2 szt, 4 szt, 6 szt",
      },
    ],
  },
  {
    id: 7,
    name: "Pedicure z pododyskem",
    items: [
      {
        id: 1,
        name: "Pedicure + Pomalowanie Hybrydą Jednolity Kolor",
        price: 190,
      },
      {
        id: 2,
        name: "Pedicure + Odżywka",
        price: 160,
      },
      {
        id: 3,
        name: "Pedicure + Zwykły Lakier",
        price: 170,
      },
      {
        id: 4,
        name: "Pedicure + French",
        price: 210,
      },
       {
        id: 5,
        name: "Pedicure Męski",
        price: 160,
      },
    ],
  },
  {
    id: 8,
    name: "Pół Pedicure",
    items: [
      {
        id: 1,
        name: "Pół Pedicure + Hybryda Jednolity Kolor",
        price: 160,
      },
      {
        id: 2,
        name: "Pół Pedicure + Zwykły Lakier",
        price: 140,
      },
      {
        id: 3,
        name: "Pół Pedicure + French",
        price: 170,
      },
      {
        id: 4,
        name: "Pół Pedicure + Odżywka",
        price: 130,
      },
    ],
  },
  {
    id: 9,
    name: "Ściąganie Hybrydy",
    items: [
      {
        id: 1,
        name: "Ściąganie Hybrydy + Kształt",
        description: "+ Hybryda Położona u Nas",
        price: 70,
      },
      {
        id: 2,
        name: "Ściąganie Hybrydy + Kształt",
        description: "+ Manicure",
        price: 130,
      },
      {
        id: 3,
        name: "Ściąganie Hybrydy + Kształt",
        description: "+ Manicure z Pomalowaniem Zwykłym Lakieremure",
        price: 150,
      },
      {
        id: 4,
        name: "Ściąganie Hybrydy z Nóg",
        price: 40,
      },
      {
        id: 5,
        name: "Ściąganie Żelu",
        price: 80,
      },
    ],
  },
  {
    id: 10,
    name: "Naprawa Paznokcia",
    items: [
      {
        id: 1,
        name: "Naprawa Paznokcia Hybrydowego",
        price: 20,
      },
      {
        id: 2,
        name: "Naprawa Paznokcia Żelowego",
        price: 30,
      },
    ],
  },
];

export const priceDataEyes = [
  {
    id: 0,
    name: "Brwi",
    items: [
      {
        id: 1,
        name: "Regulacja Brwi",
        description: "Regulacja brwi (pęseta lub wosk)",
        price: 50,
      },
      {
        id: 2,
        name: "Męska regulacja brwi",
        price: 50,
      },
      {
        id: 3,
        name: "Regulacja Brwi + Farbka",
        price: 70,
      },
      {
        id: 4,
        name: "Regulacja Brwi + Henna",
        price: 80,
      },
      {
        id: 5,
        name: "Regulacja brwi + Farbka + Wąsik",
        price: 90,
      },
      {
        id: 6,
        name: "Laminacja brwi + Farbka",
        price: 120,
      },
      {
        id: 7,
        name: "Laminacja brwi + Farbka + Botox",
        price: 150,
      },
      {
        id: 8,
        name: "Laminacja Brwi bez pomalowania",
        price: 100,
      },
      {
        id: 9,
        name: "Farbka Brwi",
        price: 40,
      },
      {
        id: 10,
        name: "Pomalowanie rzęs",
        price: 40,
      },
      {
        id: 11,
        name: "Wąsik",
        description: "Depilacja wąsika woskiem",
        price: 30,
      },
    ],
  },
  {
    id: 1,
    name: "Laminacja Rzęs",
    items: [
      {
        id: 1,
        name: "Laminacja rzęs",
        price: 130,
      },
      {
        id: 2,
        name: "Laminacja rzęs + Farbka",
        price: 140,
      },
      {
        id: 3,
        name: "Pomalowanie rzęs",
        price: 30,
      },
    ],
  },
  //  Przedłużanie rzęs
  {
    id: 2,
    name: "Przedłużanie rzęs",
    items: [
      {
        id: 1,
        name: "Przedłużanie rzęs 1:1",
        price: 130,
      },
      {
        id: 2,
        name: "Przedłużanie rzęs 1:2 (2D)",
        price: 180,
      },
      // {
      //   id: 3,
      //   name: "Przedłużanie rzęs 1:3 (3D)",
      //   price: 200,
      // },
      // {
      //   id: 4,
      //   name: "Przedłużanie rzęs 1:1 + Strzałka",
      //   price: 140,
      // },
      {
        id: 3,
        name: "Efect mokrych rzęs",
        price: 160,
      },
      {
        id: 4,
        name: "Zdejmowanie rzęs",
        price: 20,
      },
    ],
  },
  {
    id: 3,
    name: "Makijaż",
    items: [
      {
        id: 1,
        name: "Makijaż wieczorowy",
        price: 190,
      },
      {
        id: 2,
        name: "Makijaż Ślubny",
        price: 230,
      },
      {
        id: 3,
        name: "Makijaż dzienny",
        price: 150,
      },
      {
        id: 4,
        name: "Makijaż okazjonalny",
        price: 190,
      },
      {
        id: 5,
        name: "Makijaż Ślubny Próbny",
        price: 160,
      },
    ],
  },
];
